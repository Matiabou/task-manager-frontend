...
export default function LoginPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-slate-950 px-4 py-12 text-slate-100">
      <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-slate-900/95 p-8 shadow-[0_28px_60px_rgba(15,23,42,0.55)] backdrop-blur-xl">
        <div className="mb-6 space-y-3">
          <h1 className="text-3xl font-semibold text-white">{t.loginPageTitle}</h1>
        </div>
        ...
        <form onSubmit={handleLogin} className="mt-6 flex flex-col gap-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="inline-flex items-center justify-center rounded-2xl bg-emerald-500 px-4 py-3 text-base font-semibold text-white transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {loading ? (
        ...
        <p className="mt-6 text-center text-sm text-slate-400">
          {t.noAccount}{' '}
          <Link href="/register" className="font-semibold text-white hover:text-emerald-300">
            {t.signUp}
          </Link>
        </p>
      </div>
